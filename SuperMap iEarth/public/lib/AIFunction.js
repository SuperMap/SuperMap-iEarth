class AIFunction {
  ...
  postMessageFunction(event) {
    ...
    if ((!event || !event.data) && event.data.type !== "aiFunMessage") return;
    ...
    if (data && data.message == "isAllTilesLoadedReady") {
      ...
      this.maxTimeOutNumber = data.maxTimeOutNumber
        ...
        : 10000;
      // 事件监听
      this.handleAllTilesLoaded_Bind = this.handleAllTilesLoaded.bind(this);
      this.viewer.scene.layers.layerQueue.forEach((layer) => {
        layer.allTilesLoaded.addEventListener(this.handleAllTilesLoaded_Bind);
      });
  ...
  clearExitAnalyses() {
    ...
    if (this.skyline) {
      ...
      this.skyline = undefined;
      if (!this.isCoexist)
        this.viewer.camera.moveEnd.removeEventListener(this.updateSkyline_Bind);
    }
  ...
  updateShadowQuery() {
    ...
    if (!this.shadowQuery)
      this.shadowQuery = new SuperMap3D.ShadowQueryPoints(this.viewer.scene); // 初始化
