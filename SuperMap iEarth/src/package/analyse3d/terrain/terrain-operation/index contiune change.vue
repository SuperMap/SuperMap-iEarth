<template>
  ...
  <div v-if="state.operationType === 'dig'">
    <div class="row-item">
      ...
      <n-input-number style="width: 1.96rem;" :update-value-on-input="false" v-model:value="state.digDepth" :show-button="false">
    ...
    <div class="row-item" v-if="state.isPullOut">
      ...
      <n-input-number style="width: 1.96rem;" v-model:value="state.upHeight" :update-value-on-input="false" :show-button="false">
        <template #suffix>{{$t('global.meter')}}</template>
      </n-input-number>
    </div>
  ...
  <div class="btn-row-item">
    <n-button type="info" color="#3499E5" text-color="#fff" @click="Analyze" style="margin-right: 0.1rem">{{state.operationType == 'dig' ? $t('global.excavate') : $t('global.flatten')}}</n-button>
