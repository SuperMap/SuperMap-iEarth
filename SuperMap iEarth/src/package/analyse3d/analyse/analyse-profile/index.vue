<template>
    <n-scrollbar style="max-height: 3.8rem">
      <div class="row-item">
        ...
        <n-input-number style="width: 1.96rem;" v-model:value="state.startDegreesArray[0]" :show-button="false" disabled>
          <template #suffix>°</template>
        </n-input-number>
      </div>
      <div class="row-item">
        <span>{{ $t('global.startLatitude') }}</span>
        <n-input-number style="width: 1.96rem;" v-model:value="state.startDegreesArray[1]" :show-button="false" disabled>
          <template #suffix>°</template>
        </n-input-number>
      </div>
      <div class="row-item">
        <span>{{ $t('global.startElevation') }}</span>
        <n-input-number style="width: 1.96rem;" v-model:value="state.startDegreesArray[2]" :show-button="false" disabled>
      ...
      <div class="row-item">
        ...
        <n-input-number style="width: 1.96rem;" v-model:value="state.endDegreesArray[0]" :show-button="false" disabled>
          <template #suffix>°</template>
        </n-input-number>
      </div>
      <div class="row-item">
        <span>{{ $t('global.endLatitude') }}</span>
        <n-input-number style="width: 1.96rem;" v-model:value="state.endDegreesArray[1]" :show-button="false" disabled>
          <template #suffix>°</template>
        </n-input-number>
      </div>
      <div class="row-item">
        <span>{{ $t('global.endElevation') }}</span>
        <n-input-number style="width: 1.96rem;" v-model:value="state.endDegreesArray[2]" :show-button="false" disabled>
          <template #suffix>{{ $t('global.meter') }}</template>
        </n-input-number>
      </div>
  ...
  <div class="btn-row-item" style="margin-right: 0.1rem;margin-top: 0.12rem">
    <n-button type="info" color="#3499E5" text-color="#fff" class="ans-btn"
      @click="analysis">{{ $t('global.analysis') }}</n-button>
  ...
  <div v-show="state.profileInfoShow">
    ...
    <div class="chartContainer" v-show="state.gpuDomShow">
      <div style="font-size: 16px;margin-top: -5px;">{{$t('global.GPUSection')}}</div>
