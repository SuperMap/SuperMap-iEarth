<template>
    <div class="layerSeries-box">
        ...
        <n-input class="add-input-border" @input="handleChange" v-model:value="state.dataServiceUrl" type="text" style="width: 2.4rem"/>
    </div>
    <div class="row-item">
        <span>{{ $t('global.dataSourceName') }}</span>
        <n-input class="add-input-border" @input="handleChange" v-model:value="state.dataSourceName" type="text" style="width: 2.4rem"/>
    </div>
    <div class="row-item">
        <span>{{ $t('global.datasetName') }}</span>
        <n-input class="add-input-border" @input="handleChange" v-model:value="state.dataSetName" type="text" style="width: 2.4rem" />
    </div>
    <div class="row-item">
        <span> {{ $t('global.color') }} </span>
        <div class="color-pick-box" style="width:2.4rem">
    ...
    <div class="btn-row-item" style="margin-left: 0.7rem;">
        <n-button type="info" color="#3499E5" text-color="#fff" class="ans-btn" @click="singleQuery">{{ $t('global.query') }}</n-button>
    ...
    <div class="bableShadow" ref="bableQuery" v-show="state.qxModelQuery">
        <div class="row-item" style="margin-top:0.12rem">
            <span class="shadow-anaylse-pop-titie">{{  $t('global.featrueID') }}:{{ state.featureID }} - {{ $t('global.queryResult') }}</span>
            <span @click="state.qxModelQuery = false" style="margin-right:14px">X</span>
        </div>
        <div class="bable-container">
            <n-scrollbar style="max-height: 3.8rem">
                <div class="row-item" style="margin-left: 0.12rem;margin-right: 0.12rem" v-for="item in state.featureInfo">
...
function singleQuery() {
    if(state.dataServiceUrl == '' || state.dataSourceName == '' || state.dataSetName == '' ){
        message.warning(GlobalLang.singleTip_waring);
        return;
    }
    message.success(GlobalLang.singleTip_success);
