<template>
  ...
  <div class="viewshed-container">
    ...
    <!-- 附加高度 有问题 后续在解决 -->
    <!-- <sm-rowLayOut>
      <template #item-lable>{{ $t("global.additionalHeight") }}</template>
      <template #item-content>
        <n-slider
    ...
    <!-- 水平视角 -->
    <sm-rowLayOut lableWidth="1rem">
      <template #item-lable>{{ $t("global.horizontalFov") }}</template>
      <template #item-content>
        <n-input-number
    ...
    <!-- 垂直视角 -->
    <sm-rowLayOut lableWidth="1rem">
      <template #item-lable>{{ $t("global.verticalFov") }}</template>
      <template #item-content>
        <n-input-number
    ...
    <sm-rowLayOut contentMarginLeft="-0.2rem">
      <template #item-lable>{{ $t("global.hintLineColor") }}</template>
      <template #item-content>
        <div class="single-color-pick-bg">
          <sm-color-pick v-model:value="state.hintLineColor"></sm-color-pick>
        </div>
      </template>
    </sm-rowLayOut>

    <!-- <sm-rowLayOut>
      <template #item-lable>{{ $t("global.visibleBody") }}</template>
      <template #item-content>
        <n-switch
          v-model:value="state.visibleBody"
          @update:value="state.visibleColorBarShow = !state.visibleColorBarShow"
        />
      </template>
    </sm-rowLayOut> -->
    ...
    <!-- 显示可视体 -->
    <sm-rowLayOut contentMarginLeft="-0.2rem">
      <template #item-lable>{{ $t("global.visibleBodyColor") }}</template>
      <template #item-content>
        <!-- <sm-color-pick
          className="visibleAreaColor"
    ...
    <sm-rowLayOut contentMarginLeft="-0.2rem">
      <template #item-lable>{{ $t("global.hiddenAreaColor") }}</template>
      <template #item-content>
        <div class="check-color-pick">
    ...
    <!-- 动态可视域 -->
    <!-- <sm-rowLayOut  contentMarginLeft="-0.2rem">
      <template #item-lable>{{ $t("global.viewshedAnimation") }}</template>
      <template #item-content>
...
let state = reactive<stateType>({
  ...
  verticalFov: 60,
  horizontalFov: 90,
  hintLineColor: "rgba(234, 238, 22, 0.5)",
  visibleColorBarShow: false,
  invisibleColorBarShow: false,
  visibleAreaColor: "rgba(9, 199, 122, 0.5)",
...
function LEFT_CLICK(e) {
  ...
  viewshed3D.build();
  // 观察者信息记录
  state.observerInformation = p.map((num) => Number(num.toFixed(2)));
  // 添加观察者点
...
function setCarState() {
  ...
  for (let i = 1, j = points.length; i < j; i++) {
    ...
    let d: number = Cesium.Cartesian3.distance(startPoint, endPoint);
