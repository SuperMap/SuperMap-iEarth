...
class ShadowQuery {
   ...
   */
    ...
    //初始化
    init(viewer) {
        this.shadowQuery = new Cesium.ShadowQueryPoints(viewer.scene);    //创建阴影查询对象
        this.s3mInstanceCollection = new Cesium.S3MInstanceCollection(viewer.scene._context);
    ...
    updateOptionsParams(options) {
        if (!options) return;
        if (Cesium.defined(options.modelUrl)) {
            this.modelUrl = options.modelUrl;
            this.s3mInstanceCollection.setCullEnabled(options.modelUrl, true)//开启单面渲染
        };
        if (Cesium.defined(options.timeInterval)) this.timeInterval = options.timeInterval;
        if (Cesium.defined(options.spacing)) this.spacing = options.spacing;
        if (Cesium.defined(options.bottomHeight)) this.bottomHeight = options.bottomHeight;
        if (Cesium.defined(options.extrudeHeight)) this.extrudeHeight = options.extrudeHeight;
        if (Cesium.defined(options.shadowBodyShow)) this.shadowBodyShow = options.shadowBodyShow;
    }
    ...
    setShadow(val) {
        ...
        for (let i = 0; i < layers.length; i++) {
            if (val) layers[i].shadowType = Cesium.ShadowType.ALL;
            else layers[i].shadowType = Cesium.ShadowType.NONE;
        }
    }
        ...
        if (val) {
            ...
            for (let i = 0, j = this.shadowPoints.length; i < j; i++) {
                objs.push({
                    ...
                    scale: new Cesium.Cartesian3(boxWidth, boxWidth, boxWidth)
                })
            };
            if (objs.length > 0) {
