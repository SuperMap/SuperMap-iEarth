<template>
  <n-space vertical>
    <sm-rowLayOut>
      <template #item-lable>{{ locale.RollerMode }}</template>
    ...
    <sm-rowLayOut
      v-if="
        ...
        state.selectedType === 'customRoller'
      "
    >
      <template #item-lable>屏蔽方向</template>
      <template #item-content>
    ...
    <!-- <sm-rowLayOut v-if="state.selectedType != 'noRoller'">
      <template #item-lable>图层列表</template>
      <template #item-content>
        <div class="comLayerTreeBox">
          <com-layer-tree
...
import ComLayerTree from "@/package/components/common/com-layer-tree/index";
...
function createSlider(dom?: any, id?: any) {
  fdom = document.getElementById("cesiumContainer");
...
//   layers = Cesium.defaultValue(s3m, []);
//   // layers = viewer.scene.layers.layerQueue;
//   imgLayers = Cesium.defaultValue(imgs, []);
//   mvtLayers = Cesium.defaultValue(mvts, []);
// }
...
function setRollerShutterSplit() {
  ...
  switch (mode) {
    case 1:
      Cesium.BoundingRectangle.unpack([x1, 0, 1, 1], 0, scratchSwipeRegion);
      break;
    case 2:
      Cesium.BoundingRectangle.unpack([0, 0, x1, 1], 0, scratchSwipeRegion);
      break;
    case 4:
      Cesium.BoundingRectangle.unpack([0, y1, 1, 1], 0, scratchSwipeRegion);
      break;
    case 8:
      Cesium.BoundingRectangle.unpack([0, 0, 1, y1], 0, scratchSwipeRegion);
      break;
    case 15:
