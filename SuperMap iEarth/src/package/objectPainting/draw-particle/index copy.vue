<template>
  ...
  <div class="row-item" v-show="state.paramSetShow">
    <span>粒子数量</span>
    <n-input-number
...
import particles from "./js/particle-options";
import ParticleSystem from "./js/creat-particle-system";
...
function setState() {
  ...
  if (state.selectedId == 0 && state.selectedChildrenId == 1)
    state.bursts = [
      new SuperMap3D.ParticleBurst({ time: 1.5, minimum: 80, maximum: 150 }),
      new SuperMap3D.ParticleBurst({ time: 1.7, minimum: 150, maximum: 200 }),
    ] as any;
...
function addFireWork(position) {
  ...
  for (let i = 0; i < numberOfFireworks; ++i) {
    let x = SuperMap3D.Math.randomBetween(xMin, xMax);
    ...
    for (let j = 0; j < 3; ++j) {
      bursts.push(
        new SuperMap3D.ParticleBurst({
          time: SuperMap3D.Math.nextRandomNumber() * 12,
          minimum: 400,
          maximum: 400,
        })
      );
    }
    let color = SuperMap3D.Color.fromRandom(
...
watch(
  ...
  (val) => {
    if (!particleSystem.particleSystem) return;
    particleSystem.particleSystem.emitter = new SuperMap3D.CircleEmitter(
      val[0],
      val[1]
    );
